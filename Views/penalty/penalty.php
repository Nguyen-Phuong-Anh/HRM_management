        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Tên nhân viên</th>
					<th>Loại hình kỷ luật</th>
					<th>Ngày kỷ luật</th>
					<th>Hình thức kỷ luật</th>
					<th>Trạng thái kỷ luật</th>
                    <th></th>
                </tr>
            </thead>
            <tbody> 
				<?php
					...
					if(isset($_POST['search_bonus'])) {
						if(empty($_POST['khoa']) && empty($_POST['phongBan'])) {
							...
							$result = $controller->handleSearchPenalty();
                            if(!empty($result)) {
                                foreach($result as $row) {
                                    echo '<tr>';
                                    echo"<td class='table-cell'>".$index."</td>";
                                    echo"<td class='table-cell'>".$row["hoTen"]."</td>";
                                    echo"<td class='table-cell'>".$row["loaiHinhKyLuat"]."</td>";
                                    echo"<td class='table-cell'>".$row["ngayKyLuat"]."</td>";
                                    echo"<td class='table-cell'>".$row["hinhThucKyLuat"]."</td>";
                                    echo"<td class='table-cell'>".$row["trangThai"]."</td>";
                                    echo '<td><a href="?route=change_penalty&paramMKL='.$row['maKyLuat'].'" class="view" title="View Details" data-toggle="tooltip">Sửa</a></td>';
                                    echo '</tr>';
                                    $index++;
                                }
                            }
						...
						require_once('./Controllers/PenaltyController.php');
						$controller = new PenaltyController();
						$result = $controller->handleGetPenalty();	
